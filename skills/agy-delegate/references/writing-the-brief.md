...
The bridge routes automatically: each tool leads with the user's recorded `set_model` choice and falls
back down a chain of family selectors on quota exhaustion. Pass `model` or `effort` on a call only when
the human named one for this task; `agy_status` lists the labels agy offers.

A `follow_up` keeps the conversation context. Send only the delta brief.
...
The dispatch returns agy's closing response as the payload of the fenced answer. If Antigravity finishes
without a closing summary, there is nothing useful to review - and the bridge reports no touched-file
list to fall back on. The `<structured_output_contract>` block is what guarantees a report you can read.
Read that report as a claim; git and a gate re-run are the evidence.
...
on. If a premise turns out wrong while the run is live, cancel the call and re-dispatch a corrected
...
Send this as the `prompt` of a `delegate` call with `write: true` (see
[dispatch-and-poll.md](dispatch-and-poll.md)); review the result and commit it yourself (see
